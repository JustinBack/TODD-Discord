...
module.exports = {
    ...
    priviliged: false,
    execute: (message, bot) => {
        function truncate(str, n, useWordBoundary = false) {
            if (str.length <= n) {
                return str;
            }
        ...
        message.message.channel.send("Hold on... Loading!").then((msg) => {
            ...
            request("https://beta.tosdr.org/api/2/" + message.argument + ".json", function (error, response, body) {
                ...
                for (var index in json.parameters.points.slice(0, 10)) {
                    ...
                    embed.addField(truncate(pointData.title, 60), "[" + truncate(pointData.tosdr.tldr, 100) + "](https://edit.tosdr.org/points/" + pointData.id + ")", true);
                }
